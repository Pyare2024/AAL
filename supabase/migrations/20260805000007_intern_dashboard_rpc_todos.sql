-- ==============================================================================
-- UPDATE INTERN DASHBOARD SUMMARY RPC
-- Includes todo_items count for Today's Todos
-- ==============================================================================
...
BEGIN
  ...
  WHERE s.attendance_date >= COALESCE(v_joining_date::date, CURRENT_DATE)
    ...
    AND LOWER(COALESCE(s.status::text, 'live')) != 'cancelled';

  SELECT 
    COUNT(CASE WHEN r.status::text = 'present' THEN 1 END),
    COUNT(CASE WHEN r.status::text = 'late' THEN 1 END),
    COUNT(CASE WHEN r.status::text = 'absent' THEN 1 END),
    COUNT(CASE WHEN r.status::text = 'leave' THEN 1 END)
  ...
  SELECT COALESCE(r.status::text, 'not_marked') INTO v_today_status
  FROM public.attendance_sessions s
  ...
  -- 3b. Today's Todos
  -- Verify if table exists first so RPC doesn't break if todo_items is missing
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'todo_items') THEN
    ...
    ' INTO v_todo_count USING v_uid;
  END IF;

  -- 4. Recent Announcements
  SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'announcements' AND column_name = 'content') INTO v_has_content;
  IF v_has_content THEN
    EXECUTE '
      ...
      ) a' INTO v_announcements;
  ELSE
    EXECUTE '
      SELECT jsonb_agg(to_jsonb(a))
      FROM (
        SELECT id, title, description as content, created_at
        FROM public.announcements
        WHERE status = ''published'' AND deleted_at IS NULL AND (expires_at IS NULL OR expires_at > NOW())
        ORDER BY published_at DESC NULLS LAST, created_at DESC LIMIT 3
      ) a' INTO v_announcements;
  ...
  RETURN jsonb_build_object(
    ...
    'actionable_tasks_summary', jsonb_build_object(
      ...
      'today_todos_count', COALESCE(v_todo_count, 0)
    ),
...
REVOKE ALL ON FUNCTION public.get_intern_dashboard_summary() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_intern_dashboard_summary() TO authenticated;
