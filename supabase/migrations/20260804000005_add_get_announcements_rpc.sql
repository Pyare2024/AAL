...
BEGIN
  ...
  WITH filtered_announcements AS (
    SELECT 
      ...
      (SELECT count(*) FROM public.announcement_attachments aa WHERE aa.announcement_id = a.id) as attachment_count
    ...
    WHERE a.deleted_at IS NULL
      ...
      AND (
        ...
        OR
        (v_role = 'intern' AND a.status = 'published' AND (a.expires_at IS NULL OR a.expires_at > NOW()) AND public.intern_matches_announcement_target(a.id, v_uid))
      )
      -- Search Filter
      AND (p_search_text IS NULL OR a.title ILIKE '%' || p_search_text || '%' OR a.content ILIKE '%' || p_search_text || '%')
      -- Status Filter
      ...
      AND (p_problem_statement_id IS NULL OR EXISTS(SELECT 1 FROM public.announcement_targets at2 WHERE at2.announcement_id = a.id AND at2.target_type = 'problem_statement' AND at2.target_reference_id = p_problem_statement_id))
      -- Date Filters
      AND (p_date_from IS NULL OR a.published_at >= p_date_from::timestamptz OR a.created_at >= p_date_from::timestamptz)
      AND (p_date_to IS NULL OR a.published_at <= p_date_to::timestamptz OR a.created_at <= p_date_to::timestamptz)
  ),
  read_filtered AS (
    SELECT * FROM filtered_announcements
    WHERE (p_read_filter IS NULL OR p_read_filter = 'all' OR (p_read_filter = 'read' AND is_read = true) OR (p_read_filter = 'unread' AND is_read = false))
  ),
  counted AS (
